...
def test_root_dev_script_starts_all_three_services() -> None:
    ...
    assert 'command: javaExecutable' in launcher
    assert 'inkforge-core-api-0.1.0-SNAPSHOT.jar' in launcher
    ...
    assert '"apps/core-api-java", "-am", "-DskipTests", "package"' in launcher
...
def test_e2e_uses_current_project_and_writing_entry_points() -> None:
    ...
    assert "本章待确认 0 项" not in writing
    assert "await expect(pendingTrigger).toHaveCount(0)" in writing
    assert 'getByText("暂无待确认变更。", { exact: true })).toBeVisible()' in writing
