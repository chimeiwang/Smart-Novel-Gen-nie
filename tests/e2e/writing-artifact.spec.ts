...
test("生成正文进入固定审核栏，刷新恢复主会话与待确认产物后可以应用", async ({ page }) => {
  ...
  expect(reviewLayer).toBeGreaterThan(userLayer);
...
test("普通问答完成后可以恢复双方消息", async ({ page }) => {
  ...
  await expect(page.getByText("模拟模型已完成本轮处理。", { exact: true }).first()).toBeVisible({
    timeout: 60_000,
  });

  await page.reload();
  await expect(page.getByRole("button", { name: /历史对话\s*：开始写这一章前/ })).toBeVisible();
  ...
  expect(session.messages[1]?.content).toContain("模拟模型已完成本轮处理。");
});
