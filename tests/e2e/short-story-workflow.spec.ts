...
test("空篇幅参考可从灵感多次改纲、完整返工并批准为唯一正文", async ({ page }) => {
  ...
  await expect(page.getByLabel("篇幅参考（可选）")).toHaveValue("");
  ...
  expect(firstDraft.payload.metadata.targetWordCount).toBeNull();
  ...
  const firstDraftLength = countShortStoryTextLength(firstDraft.payload.content);
  expect(firstDraft.payload.metadata.actualWordCount).toBe(firstDraftLength);
  expect(firstDraftLength).toBeGreaterThanOrEqual(6_000);
  expect(firstDraftLength).toBeLessThanOrEqual(80_000);
  ...
  await page.getByRole("button", { name: "完整初稿", exact: true }).click();
  await expect(page.getByText(`实际 ${firstDraftLength} 字`, { exact: true })).toBeVisible();
  await expect(page.getByText(/篇幅参考约 \d+ 字/u)).toHaveCount(0);
  ...
  const revisedDraftLength = countShortStoryTextLength(revisedDraft.payload.content);
  expect(revisedDraft.payload.metadata.actualWordCount).toBe(revisedDraftLength);
  expect(revisedDraftLength).toBeGreaterThanOrEqual(6_000);
  expect(revisedDraftLength).toBeLessThanOrEqual(80_000);
  ...
  const appliedLength = countShortStoryTextLength(appliedChapter.content);
  expect(appliedLength).toBe(revisedDraftLength);
  expect(appliedLength).toBeGreaterThanOrEqual(6_000);
  expect(appliedLength).toBeLessThanOrEqual(80_000);
...
test("非空篇幅参考只作倾向并可生成完整正文后正式应用", async ({ page }) => {
  test.setTimeout(180_000);

  const targetWordCount = 20_000;
  ...
  await startShortStoryOperationWithApi(
    ...
    "write_short_story",
    targetWordCount,
    "根据已批准大纲一次生成完整正文并完成全稿审核，篇幅参考只作倾向。",
  );
  ...
  const draftLength = countShortStoryTextLength(draft.payload.content);
  expect(draft.payload.metadata.actualWordCount).toBe(draftLength);
  expect(draftLength).toBeGreaterThanOrEqual(6_000);
  expect(draftLength).toBeLessThanOrEqual(80_000);
  ...
  const chapterLength = countShortStoryTextLength(chapter.content);
  expect(chapterLength).toBe(draftLength);
  expect(chapterLength).toBeGreaterThanOrEqual(6_000);
  expect(chapterLength).toBeLessThanOrEqual(80_000);
