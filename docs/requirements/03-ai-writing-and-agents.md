...
2026-09-04 自然入口迁移正在实施：共享意图输入/结果/澄清快照、Agent 的单次 resolve_intent 执行器和
Core 的冻结待解析计划已实现；Core 的同 Run 续接、澄清受理及普通 Web/CLI 入口尚未接通，当前普通聊天
仍使用下述 V1 入口。详细范围与逐项验收见 `docs/specs/2026-09-04-durable-natural-language-entry.md`，
不能把基础层测试或系统 Profile 的 supported 标志当作用户入口已完成。
