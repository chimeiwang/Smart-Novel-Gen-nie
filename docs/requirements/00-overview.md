...
本文档基于当前 FastAPI 服务、数据库结构契约、生成的 OpenAPI 客户端、Next.js 页面、Agent 契约和测试反向整理。它只描述当前已实现的产品需求。
...
- 项目以现有 PostgreSQL 和只读 `schema-contract.json` 为数据结构事实来源，重构期间禁止修改 schema。
...
| 项目与章节 | 小说创建、章节列表、章节编辑、自动保存、送审、完成、字数统计 | 工作台首页、工作台、Core API |
...
| AI 写作会话 | 会话 CRUD、消息持久化、流式 Agent 输出、恢复、继续修改草案 | 写作面板、Core API 与 SSE |
