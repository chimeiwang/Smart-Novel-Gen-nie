...
状态：仓内、真实开发及生产验收已完成，两份固定 CLI 包已同步；48条旧执行保全退出、正式库两次幂等迁移、
75表contract复验、联合drain和指定账号canary均已完成。`8a1324c` 已正式部署并通过all门禁，已开放业务的新请求
全量走V2，V1 fresh关闭，生产视频仍关闭。问答及同请求重放、审阅、规划复审／弃用／采用、真实CLI中断重连及取消均已验证。
8条Run包含4completed／3failed／1cancelled；失败和全部82.10684积分真实费用保留，未伪造成功或重复计费。
原小说成果保全，旧成果公共读取30次通过；精确9条已投递旧Outbox在验收期间超过原7天保留期，按下文单列，
其余旧行逐值保全。既有V2事实永久禁止DDL rollback和V1-only回滚。
...
正式维护的锁权限：生产关闭的视频表归现有 `postgres` 管理，应用角色只有读取权限，不能取得全表保全事务的
`SHARE ROW EXCLUSIVE` 锁。不得为此给应用增权或改变表所有者。具名维护可由已有服务器 root 经本机
`runuser -u postgres` 只执行冻结归档 helper 生成的原始 `apply` 事务，数据库仍精确为 `novelwriter`；
清单、备份、共享发布锁、CAS、全表指纹及事务后独立复验全部保留。只读盘点／备份／复验仍用原应用连接。
维护包装器须校验该 helper 文件的已提交 SHA256，不接受 SQL、数据库、用户或任意命令参数，不授予常驻权限。
...
长时间canary的逐行复验不应把既有Outbox终态7天过期规则当作作品丢失：本轮只允许对精确9条已published旧事件
单列留存例外，原备份完整行hash须与canary前基线相同，且其7天到期时刻处于本次采样窗口内；其余74表及Outbox
其他旧行仍逐值保全。不得整表豁免、不允许内容变化或未投递事件缺失，不重新取基线、不恢复或删除数据库记录。
最终报告必须明确9条历史事件缺失，不能写全部原行仍在；这只是用户允许旧执行历史清理的证据分类，不新增清理行为。
...
生产整章审阅 canary 已按原规则以 `MODEL_STRUCTURED_OUTPUT_INVALID` 收敛，问答及章节规划的真实模型
调用已成功。该总错误码不能区分空输出、JSON 解析失败或 Schema 不符；DeepSeek V4 的安全细分诊断
仅存在于本次调用结果内，V2 不走 V1 observer，因此不能倒推本次失败原文或直接认定配置有误。

仅在 Executor 将结构化输出诊断转成上述失败时增加一条现有 logging warning，包含精确 Run ID、
Step ID、冻结 outputSchema.name、固定诊断 code 和白名单 keyword；缺少输出且无诊断时固定
`missing_output/content`。不记录 jsonPointer、用户字段名、正文、输入、坏输出、原始异常或凭据，
未知 keyword 只记 `unknown`。日志不进入回调或 resultHash，不改变原终态、错误分类、用量、计费、
Prompt、Schema、registry、重试策略或模型调用数，也不回放已失败 Run。
