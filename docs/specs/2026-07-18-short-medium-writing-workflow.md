...
- 正文每次运行只进行一次完整生成，不逐节生成、不自动续写、不拼接截断内容。
...
- 每次 `write_short_story` 只执行一次正文模型调用，并要求模型返回带现有完整正文边界标记的单一完整稿件。
...
- [ ] 过期 `expectedRevision` 被拒绝，用户修改要求原文可从消息、命令和 revision diff 追溯。

### 整稿与审核

- [ ] 大纲批准前不能生成正文。
- [ ] 每次运行只进行一次完整正文生成，不逐节、不续写、不拼接。
