...
- 前端必须把 `revision_focus` 中保存的每轮用户修改原文与对应 ReviewArtifact revision 组合为连续的改纲对话；大纲可修改时在同一对话底部继续输入，批准或生成正文后仍可只读回看，不能只提供一次性文本框和彼此割裂的版本列表。
...
- reviewer 的执行契约只有一个终止工具 `submit_evaluation`，Runtime 必须在模型请求中显式要求调用该工具，不能只依赖提示词；对于思考模式不支持 `tool_choice` 的真实供应商，这类单一终止工具请求必须关闭思考模式后再强制调用，其他创作请求仍保留原模型模式。缺少或非法的结构化结论仍按失败关闭处理。
- reviewer 可以读取本轮用户修改原文作为验收背景，但末条用户消息必须是固定的“审核当前 Core 权威草案”；修改要求必须明确标注为要求而非正文事实，且排在当前完整草案之前，避免模型把修改要求误判为草案现状。
- 首稿任一审核要求修改时，主责写作 Agent 只自动执行一次完整返工。
- 自动返工调用和草案保存都必须使用主责“写作”身份；前一轮 reviewer 留下的 `activeAgent` 不能泄漏到返工保存权限校验。
- 自动返工后的新整稿再次依次执行编辑、校验两轮审核。
- 第二轮仍有 `revise` 或 `block` 时停止自动循环，将完整草案、两轮结论和待处理问题交给用户。
- 因此一次用户发起的整稿运行最多有两个整稿生成轮次；每个轮次都从完整权威输入生成一篇完整稿件，不能把返工当作续写或片段拼接。
...
- 中短篇大纲或正文的 `revise` 决策一旦进入持久命令，无论失败由 Agent 回调还是 Core 投递器观察到终态失败，都必须把精确绑定的 Artifact 恢复为 `awaiting_user`，并恢复可继续决策的稳定快照；不得留下 `draft + error` 的不可操作状态。
...
- [x] 中短篇缺少显式 Operation 或组合不匹配时，模型调用次数为零。
- [x] 纯灵感请求确定进入 `develop_short_outline`，不依赖关键词分类。
- [x] 上下文严格遵循“直接编辑 → 修改原文 → 锚点 → 当前大纲 → 灵感 → 最近对话”优先级。
- [x] 几十轮历史仍可查询，但不会全部进入模型输入。
- [x] 中短篇复用 WritingSession，可新建、切换并继续历史对话；右栏只承担消息与输入。
- [x] 大纲与正文使用独立项目级版本号；显式附件或“大纲 v2/正文第 3 版”会在模型调用前解析为精确 revision/hash，缺失时禁止回退最新版。
- [x] 已采用大纲产生新待确认版本后，原采用版本仍可作为正文来源；新对话改正文继续写入同一项目级正文版本链。

### 大纲与版本

- [x] 首次返回完整可读大纲，不包含固定节数、节长或每节字数。
- [x] 局部修改按稳定 ID 合并，未涉及分节逐值保持不变。
- [x] 用户可无限次改纲、直接编辑、批准、查看历史和恢复旧版。
- [x] 内容变化才增加 revision；状态切换和幂等重试不增加。
- [x] 直接编辑先保存新 revision；恢复历史复制为更大的新 revision。
- [x] `approve`、`revise`、`discard`、直接编辑和恢复都校验 `expectedRevision` 并拒绝过期请求；用户修改要求原文可从消息、命令和 revision diff 追溯。
- [x] Web 以对话形式展示每轮改纲原文及对应完整大纲版本，并允许在同一位置连续提交下一轮修改。

### 会话生命周期补充（2026-07-19）

- “开始新对话”只在浏览器内切换为未持久化的空白对话，不得在用户尚未发送消息时创建 WritingSession。
- 用户首次发送消息时，前端先创建 WritingSession，再以该会话 ID 启动写作运行；创建成功后的消息和任务继续按现有契约持久化。
- 历史对话只展示至少包含一条消息的会话。此前已经产生的空会话保留在数据库中但不进入工作区历史列表，不执行破坏性清理。
- 选择历史对话后必须立即关闭历史列表、清空上一段对话内容并显示读取状态；请求完成后只展示所选会话，过期请求不得覆盖当前会话或提前结束当前加载状态。
...
- [x] 批准后完整正文写入唯一 Chapter；只有校验明确通过才标记全稿审核覆盖。
