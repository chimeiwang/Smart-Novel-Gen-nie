...
- 后续变更：本规格的“禁止并行执行多个模型任务”部署约束已由 `2026-08-06-agent-bounded-concurrency.md` 取代；持久恢复与监督原则继续有效。
...
写作命令和旧任务恢复也必须消费 Agent 返回的真实 job 状态：

- `pending` 命令首次投递，以及超过时限仍为 `submitted/processing` 的命令，都使用命令 ID 查询同一个稳定 job；
- Agent 返回 `queued/running` 时，命令保持或推进为 `submitted`，等待正式回调；
- Agent 返回 `completed/failed/cancelled`，但 PostgreSQL 尚未收到正式终态回调时，在同一事务锁定命令与任务并把两者幂等收敛为失败；已经终态的命令或任务保持不变；
- 没有活动命令的旧 `active/waiting_call` 任务按稳定 job ID 对账；终态收敛必须同时匹配领取时的 `phase` 与 `graphStateJson`，新快照或新阶段不得被旧对账结果覆盖。
...
- 写作命令在首次落库状态更新失败、正式回调持续失败或 Core 重启后，能从 `pending/submitted/processing` 重新查询稳定 job 并收敛；旧任务的新阶段或新图快照不被延迟终态覆盖。
