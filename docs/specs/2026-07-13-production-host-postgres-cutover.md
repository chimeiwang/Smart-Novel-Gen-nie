...
生产服务器生成两组新的 Ed25519 服务身份材料：Core 到 Agent、Agent 到 Core。私钥归属容器 UID/GID `10001:10001` 且权限设为 `600`；`infra/secrets` 目录归属 `root:部署用户组` 且权限设为 `710`，部署用户只能检查已知文件，不能列出目录或读取私钥；`.env` 归属 `root:部署用户组` 且权限设为 `640`，使 SSH 部署用户只能读取、不能修改；公钥材料权限设为 `644/root`。密钥只保存在服务器 `infra/secrets`，不进入 Git、镜像或日志。
