...
生产服务器生成两组新的 Ed25519 服务身份材料：Core 到 Agent、Agent 到 Core。私钥归属容器 UID/GID `10001:10001` 且权限设为 `600`，`.env` 权限设为 `600/root`，公钥材料权限设为 `644/root`。密钥只保存在服务器 `infra/secrets`，不进入 Git、镜像或日志。
