...
- 令牌包含 `iss`、`sub`、`aud`、`scope`、`task_id`、`run_id`、`novel_id`、`jti`、`iat` 和 `exp`。
- 令牌有效期 2 分钟，最大不超过 5 分钟。
- Redis 记录已消费的高风险 `jti`，生存时间等于令牌最大寿命。
...
- 内部接口同时验证网络来源和服务 JWT，不能只依赖 Docker 网络。
