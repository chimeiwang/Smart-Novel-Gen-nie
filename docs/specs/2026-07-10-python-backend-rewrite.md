...
- 根目录 `npm run dev` 必须一次启动 Next.js、Core API 和 Agent Service，读取不提交仓库的 `.env.local`。
- 本地没有 Nginx，Next.js 仅在开发模式把浏览器 `/api/*` 请求代理到 `CORE_API_INTERNAL_URL`；生产仍由 Nginx 作为唯一公网入口。
- 通过 `http://127.0.0.1:43119` 访问本地页面时，客户端开发资源必须可加载并完成页面激活；Next.js 开发服务器允许的来源仅限显式本地地址 `127.0.0.1`，不得使用通配符或放宽到其他来源。
- 本地默认允许使用 fake 模型，但 PostgreSQL 和 Redis 仍由开发者显式配置，不自动创建、迁移或修改数据库结构。
