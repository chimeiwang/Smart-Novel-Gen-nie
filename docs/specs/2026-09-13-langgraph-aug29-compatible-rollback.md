...
状态：兼容版本首轮发布成功；线上只读验收发现普通查询依赖未开放视频列，新任务已重新关闭，正在修复并复验。
...
首轮上线后 V1 任务 GET 查询使用生成表的全字段投影，错误包含生产尚无的 `ReviewArtifact.videoEpisodeId`。
生产 schema 门禁按已开放能力投影，视频能力关闭，因此门禁通过不能证明普通写作 SQL 不依赖视频字段。
修复只把写作、审核及中短篇的 ReviewArtifact 查询改为显式非视频字段清单，保留完整 payload、Diff、
来源、revision 和审核状态；禁止为通过查询执行生产 DDL。隔离 Compose 增加缺列夹具，真实复现旧镜像
GET 500 后再验证新镜像的完整草案闭环，不能继续用开发数据库超集作为生产兼容证据。
