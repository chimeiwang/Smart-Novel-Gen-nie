...
状态：脚本与演练完成；首次正式执行因现网旧镜像不兼容已精确回滚，等待兼容版本先行发布
日期：2026-08-23

## 1. 背景与现状

章节影视化、视觉设定版本和逐镜提示词链路已在服务器端 `novelwriterdev` 开发库完成真实验收，
但正式库 `novelwriter` 仍保持迁移前结构。应用以关闭视频预览开关的 schema profile 运行，因此正式
服务当前不依赖这些表；后续要发布同一套视频功能，必须先把已经验证的结构以受控方式晋升到正式库。
...
6. 迁移保持与应用发布解耦；本次不打开视频预览开关，不部署新镜像。
...
本次全部是 additive DDL：创建表、列、索引、唯一约束、检查约束和外键。正式库当前没有视频域表，
因此不会发生视频行回填。共同表上的新列均可空；现有 `ReviewArtifact` 行继续合法。`Novel` 与
...
7. 正式环境视频开关保持关闭，迁移过程不重启或部署应用。

## 9. 2026-08-23 首次正式执行结论
...
- 正式库当前尚未保留视频结构。再次执行必须先完成兼容 Core 发布，不能把“SQL 曾成功”误写成
  “正式晋升已完成”。完整证据见 `docs/audits/2026-08-23-production-video-schema-promotion.md`。
