...
2026-08-10 的界面复核进一步明确：中央正文始终常驻；左侧仅以同一组分段按钮在“章节”和“创作资料”
之间切换导航内容；右侧只保留聊天。创作资料详情以及审核与确认不再占据主画布或右侧栏，统一使用同一套
全屏遮罩弹窗基础样式承载。
...
- AI 会话已有历史对话、新建/删除/恢复、消息编辑/删除/复制/重试、任务建议、SSE 状态、流程日志、
  Artifact 托盘和审核弹窗。
- 当前实现曾在右侧聊天下方堆叠审核栏；复核后确认该区域应移除，审核与确认统一进入弹窗。
...
2. 让“章节”和“创作资料”成为左侧栏中两个平级分段按钮，复用原“AI 创作 / 阅读与小修”的切换样式。
...
创作资料详情、待确认列表和审核详情均通过 body portal 弹窗显示，不增加第四栏，也不在右侧栏内部上下分栏。
```
...
“章节”和“创作资料”是两个平级分段按钮，不把创作资料放在章节之下，也不把角色、势力、大纲等
子项与具体章节混成同一层列表。它们使用原“AI 创作 / 阅读与小修”的紧凑分段切换视觉；旧的
“AI 创作 / 阅读与小修”按钮从左栏移除，中央始终使用当前可编辑的章节画布。

```text
[ 章节 ][ 创作资料 ]

章节分段：第一章 / 第二章 / ＋ 新增章节
资料分段：角色 / 地点 / 势力 / 物品 / 术语 / 故事背景 / 世界设定 /
          大纲 / 剧情进度 / 故事进展 / 作品圣经 / 文风 / 参考资料
```

两个按钮互斥切换下方导航内容，不使用折叠根节点。章节列表继续使用现有新增、切换、保存屏障、状态、字数
和 Beat Plan 摘要逻辑；创作资料继续使用现有分类、延迟加载、错误重试和失效刷新逻辑。

### 中央主画布

- `ChapterEditor` 始终显示，不再因打开创作资料而被替换或隐藏。
- 点击资料子项时，在共享弹窗中显示现有完整资料详情组件，而不是在左栏复制简化表单或占用中央画布。
- 打开资料弹窗前继续刷新章节自动保存；失败时不打开弹窗并显示现有错误。
- 关闭资料弹窗后继续保留当前章节、编辑状态和自动保存协调器。
- `studio`、`reading` 和 `library` 旧 URL 继续可解析为初始焦点，旧链接不失效；页面不再显示三项顶部模式开关。

统一工作台只保留当前“AI 创作”下的章节编辑表现，删除左栏“AI 创作 / 阅读与小修”二级切换。章节状态、
自动保存、送审、退回、完成、重新编辑、进展和终检入口仍保持现有行为。
...
右侧栏只显示聊天，不再渲染“审核与确认”竖向分区。聊天头部保留待确认计数和入口；点击后打开待确认列表
弹窗，再点击具体 Artifact 打开审核详情弹窗。资料详情弹窗与审核弹窗复用统一的 `WorkspaceDialog` 基础壳：
同一遮罩、圆角、边框、标题栏、关闭按钮、滚动区和焦点语义；资料详情与审核内容分别使用宽度和内部布局变体。
...
- 左栏“章节”和“创作资料”使用同一组平级分段按钮，切换下方导航内容。
- 中央正文始终可见，右栏只显示聊天，不存在审核竖排分区。
- 点击任一创作资料项打开共享样式的资料详情弹窗，关闭后正文和聊天状态不变。
...
1. 左栏只显示“章节 / 创作资料”平级分段按钮，不再显示“AI 创作 / 阅读与小修”二级切换或折叠根节点。
2. 中央章节正文与右侧聊天始终同时可见；右侧不再上下堆叠审核栏。
3. 创作资料详情、待确认列表和审核详情均使用共享弹窗基础样式；资料弹窗保留全部原有编辑能力，审核弹窗
   保留完整 Diff、批准、返工和丢弃能力。
4. 除布局与入口位置外，章节、资料、聊天、SSE、Artifact 和审核功能行为不变。
5. 选中文字本身不会把内容交给 AI；只有用户点击明确操作并发送要求后才创建任务。
6. 正文、总纲和大纲节点内容均可提交一个连续选区进行修改。
7. Agent 只返回 replacement，Core 负责拼接，选区外文本逐字不变。
8. 所有修改均经过完整 Diff、复审和 ReviewArtifact 用户决定；不能直接覆盖正式内容。
9. 来源变化时明确冲突，不自动变基、不模糊定位、不部分应用。
10. 不新增项目级会话、项目级 Agent 锁或其他业务语义。
11. 不修改 PostgreSQL schema，现有旧 URL 和未携带选区的普通长篇请求保持兼容。
