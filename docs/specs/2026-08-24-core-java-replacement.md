...
[2026-09-12 退役规格](2026-09-12-java-only-core-contract-and-python-retirement.md) 接管；
...
- 该回归同时发现 Web 把百分制 `scoreOverall=87` 错误拼接为 `87/10`。共享质量报告与持久化契约始终是
  ...
  先红后绿的前端测试锁定。
- 提交 `b8722200030c934266429b01279acc75f3b5b047` 的 GitHub Actions #117 已通过 9 分 36 秒的完整 CI，
  Web 镜像、deployment source bundle 和远端部署均成功，deploy job 用时 5 分 53 秒；同一生产报告刷新后
  原值显示为 `综合 87/100`，状态、报告、余额和登录态均保持稳定，公网 readiness 继续全部为 `ok`。
