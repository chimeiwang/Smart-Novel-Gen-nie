...
> **架构重构状态（2026-06-09）**：已完成 Phase 1-7 重构，详见 `docs/AGENT_REFACTOR_EXECUTION_PLAN.md`。
> 当前版本 v6.0（AgentDefinition + AgentRunner + 质量检查服务端化 + 写作闭环工作流）。
> 遗留问题追踪：`docs/REFACTOR_ISSUES.md`。
...
- 已有章节质量检查队列：一致性校验、同步设定、商业性评审、技法评审。
...
- “每完成 N 章自动唤醒设定 Agent”还没有做成可配置规则。
...
- 商业性评审报告已保存到 `ChapterQualityCheck.result`，六项评分已拆成可查询字段。
...
3. [已完成] 增加完成后检查队列：一致性校验、同步设定、商业性评审、技法评审均可执行；报告会保存到检查项。
4. [待完成] 增加自动触发规则：每完成 N 章，建议触发“同步设定”；每章完成后默认触发“校验 + 商业性评审”。
5. [已完成] 保存每章质量报告，避免只在聊天里看一次就丢失。

验收标准：

- 用户完成章节后，前端能看到待执行检查列表。
- 用户可逐项运行、跳过、确认 Agent 建议。
- 设定同步不覆盖核心画像，只更新阶段事实和角色经历。
