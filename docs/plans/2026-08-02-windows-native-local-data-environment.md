...
- 安装 Conda 环境 inkforge-data：PostgreSQL 14.23、pgvector 0.8.0。
...
### 任务 3：一次性复制远程 PostgreSQL

- 用现有远程 DATABASE_URL 执行本地 pg_dump，只读导出。
- 恢复到空的本地数据库，不复制远程 Redis。
- 核验 schema 指纹、目标小说和实际活动任务数量。
- dump 验证完成后保存在 Git 忽略目录，命令和日志不打印密码。
...
- Ctrl+C 后确认数据服务继续运行。
- 再次启动确认不重复创建实例。
- 验证 Web、Core、Agent 健康，登录并打开目标小说。
