...
个人项目发布范围：`docs/specs/2026-09-04-personal-durable-agent-release-scope.md`
...
- [x] 一致性终检仓内实现、全仓门禁与隔离跨进程验收；正常报告和独立格式纠正均通过，首次 Failure 的
  Agent 重启重放不重复模型或结算。该阶段仓内 17/21，严格区分执行失败与内容结论；见
  `docs/specs/2026-09-05-durable-consistency-quality.md`；
- [ ] 一致性终检真实供应商与真实开发/生产验收；本机固定 JAR 和活动 Skills 未随源码更新；
- [x] 文风画像仓内实现、全仓门禁与独立进程验收：整套五个耐久 Step，首节终态重启重放不重调，单节重做
  保留其余字段，原纯文本与完整来源/字数语义不变；新 V2 不创建旧任务影子行。当前仓内 18/21；见
  `docs/specs/2026-09-05-durable-style-portrait.md`。跨进程发现并修复控制字符 canonical 转义差异，未改写原文；
- [ ] 文风画像真实供应商与真实开发/生产验收；固定 CLI 和活动 Skills 未随源码更新；
- [x] RAG embedding 与索引回调的仓内实现、全仓门禁和独立进程验收：11 块两批，首批 journal 重启
  不重调，完整索引原子物化与零用户积分扣费通过；当前仓内 19/21，见
  `docs/specs/2026-09-05-durable-rag-embedding.md`；
- [ ] RAG 实际供应商与真实开发/生产验收；固定 CLI 和活动 Skills 未随源码更新；
- [ ] 开发环境章节影视化两个模型工作流；
...
- [ ] 在个人维护窗口完成 V2-aware route-off 应用回滚与 V1/V2 联合 drain 演练。
...
- [ ] 生产备份与恢复清单验证；
- [ ] 在开发环境完成迁移、rollback、contract 复验、真实 2C2G 和公共 CLI 真实供应商 canary；失败后验证
  route-off 清理，不能用本地 Fake Provider 代替；
