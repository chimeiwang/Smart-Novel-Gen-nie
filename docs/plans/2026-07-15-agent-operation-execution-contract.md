...
- [ ] **Step 4: 在 Runtime 接收正文和工具前检查完成原因**
...
- [ ] **Step 1: 写 Operation 契约和工具交集失败测试**
...
BUILDER_TOOLS = frozenset({
    "start_update_builder", "append_update_batch", "append_outline_tree",
    "put_update_text_block", "put_update_item_text_block",
    "put_update_item_text_blocks", "finish_update_builder",
})
```

每个 Operation 在定义中声明精确并集；`plan_chapter` 的唯一控制工具为 `submit_beat_plan`，`write_chapter/rewrite_scene` 的唯一控制工具为 `begin_artifact_output`，`answer_question/review_chapter` 无产物控制工具。

- [ ] **Step 4: 实现 ToolRegistry 三层交集**
...
- [ ] **Step 5: 改造 AgentRunRequest 和 Runner**
...
- [ ] **Step 6: 更新所有 AgentRunRequest 调用和测试 fixture**
...
- [ ] **Step 4: 让完整 Core 上下文只作为 runtime-only 状态存在**
...
- [ ] **Step 5: 实现 contextStrategy 投影**

`build_operation_context()` 接收完整 Core context 和 `OperationDefinition`，只输出不含 `userMessage/conversationHistory/graphState/activeArtifact` 的资料投影。`brief/lore/outline/chapter/review` 分别选择规格列出的摘要；不得把整个 `workspace` JSON 原样返回。
...
- [ ] **Step 6: 拆分 Core 当前用户消息并删除 parent graph 重复追加**
...
- [ ] **Step 1: 写模式传递、rewrite-only 和产物拒绝失败测试**
...
- [ ] **Step 5: 所有 revise 归一为完整 rewrite**
...
- [ ] **Step 6: 分离 reviewer/reviser 动态上下文**
...
- [ ] **Step 1: 写完整 activeArtifact、水合和稳定决定失败测试**
...
- [ ] **Step 3: 扩展 Core 内部 activeArtifact**
...
- [ ] **Step 4: 实现 ArtifactHydrationPort**
...
- [ ] **Step 5: 在 WritingJobHandler 恢复前水合**
...
- [ ] **Step 1: 写严格报告、校验 Agent 和持久映射失败测试**
...
- [ ] **Step 4: 迁移 QualityJobHandler 到校验 Agent**
