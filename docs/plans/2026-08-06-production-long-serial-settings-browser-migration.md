...
用户已明确将 Task 4 的结构化设定写入方式由 Web UI 改为生产 PostgreSQL 单事务。执行时先备份源、目标相关行，再按“地点 -> 势力 -> 人物 -> 人物关系”建立新 ID 映射；背景、世界设定和作品圣经只核对现有目标记录。Task 5 因源参考资料和应用文风均为空而保持无操作，Task 6 的公开 CLI 回拉验收不变。
