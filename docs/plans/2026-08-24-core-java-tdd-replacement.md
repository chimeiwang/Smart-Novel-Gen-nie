...
- [x] 再次提交、推送和部署后，用新质量运行回读 PostgreSQL 权威完成态，并确认相关真实流程未再复现 h2c、
  `AsyncRequestTimeoutException` 或质量任务导致的消费者意外退出症状。

**验收：** 不改数据库、OpenAPI、计价和模型策略；生产真实浏览器中的三类 Agent 任务可以从耐久待投递事实
进入 Python Agent 队列，SSE 跨过 30 秒后保持有效或按权威终态关闭。

## Task 18：移除标准部署对生产服务器 GitHub 出网的依赖

- [x] 用两次失败部署证明镜像上传瞬时中断与服务器 GitHub HTTPS 连续超时是两个独立故障；
- [x] 先写红灯架构测试，冻结精确 bundle、固定主机身份、原子上传、SHA 绑定和临时文件清理契约；
- [x] 让 GitHub Actions 从已 checkout 的目标提交创建并验证 Git bundle，通过 SSH 上传具名临时文件；
- [x] 让生产部署脚本优先从 bundle fetch 并复核 `DEPLOY_SHA`，无 bundle 时保留现有 origin 获取兼容路径；
- [x] 运行 Shell 语法、部署架构测试、Compose 安全测试与完整相关回归；
- [x] 提交、推送和部署后确认流水线成功，再执行 Task 17 的生产质量终检验收。

**验收：** 标准生产流水线不要求服务器访问 GitHub；不放宽 SSH 主机身份、不传输秘密、不在 SHA 校验前
触碰生产容器；bundle 成功或失败均不残留本次具名临时文件。

## Task 19：修正一致性终检百分制展示

- [x] 用生产完成结果确认后端权威分值为 87，页面错误显示为 `87/10`；
- [x] 先补红灯测试，锁定总分 `/100` 和等比例 50/70 色阶；
- [x] 分离一致性百分制总分与历史商业性十分制维度，不修改 API 或持久化数据；
- [x] 运行 Web 全量测试、TypeScript、ESLint 和生产构建；
- [x] 提交、推送、部署后在同一生产报告上确认显示为 `综合 87/100`。

**验收：** 生产页面原值展示一致性百分制；历史商业性字段继续按十分制显示；不重新触发模型、不改质量结果。
