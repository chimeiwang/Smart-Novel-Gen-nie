...
> 状态：已完成，归档于 2026-07-30。当前事实以代码、测试和 `docs/specs/2026-07-30-short-medium-writing-workflow.md` 为准。

> **供 Agent 执行：** 必须使用 `subagent-driven-development`（推荐）或 `executing-plans`，逐项实施本计划。步骤使用复选框语法记录状态。

**目标：** 让中短篇正文首次生成与基于当前版本的全文修订使用同一套通用、可验证的创作契约，并保持一次任务只产生一个完整候选版本。

**架构：** 保留现有 `generate_manuscript`、不可变 `baseContent`、来源蓝图和 `userInstruction` 契约；仅在 Agent Service 的消息装配阶段根据是否存在基础正文区分“首次创作”和“全文修订”。通用约束集中在正文 Operation brief，不增加 API、数据库字段、Agent 或模型调用。

**技术栈：** Python 3.12、Pydantic、pytest、Ruff、Mypy

---

### 任务 1：收敛正文提示词并验证生成与修订语义

**涉及文件：**
- 修改：`docs/specs/2026-07-30-short-medium-writing-workflow.md`
- 修改：`apps/agent-service/src/inkforge_agents/jobs/short_medium.py`
- 测试：`apps/agent-service/tests/short_medium/test_graph.py`

- [x] **Step 1: 写首次生成和全文修订的失败测试**
...
预期：两个测试因当前 brief 仍包含“第 1/1 段”且没有全文修订契约而失败。

- [x] **Step 3: 用单一正文 brief 实现最小改动**
...
预期：全部退出码为 `0`，无失败、Ruff 错误或 Mypy 错误。

- [x] **Step 6: 做一次真实版本化修订测试**
