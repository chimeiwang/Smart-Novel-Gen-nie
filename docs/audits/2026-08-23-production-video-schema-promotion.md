...
随后发布兼容迁移前后两种结构的 Core 版本，再重新执行同一个前向脚本。第二次执行完成后，正式库
保留 69 表目标结构，运行中镜像的 full schema guard 与默认 readiness 均通过；视频功能开关继续保持
关闭。
...
`without_video_preview` 投影恰为 44 表，与正式库基线 0 项差异。该修正已随
`ecfc1e968539c64bfc685d2528c8083d210f3022` 部署。

## 2. 结构差异

| 指标 | `novelwriterdev` | 正式库迁移前/首次回滚后 | 正式库最终状态 |
| --- | ---: | ---: | ---: |
| 表 | 69 | 44 | 69 |
| 视频表 | 25 | 0 | 25 |
| 枚举类型 | 22 | 22 | 22 |
...
```text
/srv/backups/inkforge/inkforge-20260823T071415Z/database.dump
```

- 大小：18,090,273 字节；
- SHA-256：`fa3c873def998f572aaf9222dcc5765eb827b978f7693c07fa2e36522736d815`；
- `sha256sum -c`、`pg_restore --list` 和完整恢复演练均通过；
- 恢复后计数：小说 192、章节 221、审核记录 215；
- 最终迁移日志 `migration-20260823-final.log` SHA-256 为
  `68185435baa7952dff476d890b6d925f8125d1c1d42b13c3f98b4b8f67b526e0`。
...
- `main` 提交：`ecfc1e968539c64bfc685d2528c8083d210f3022`；
- 运行中 Core 镜像：`inkforge-core-api:ecfc1e968539c64bfc685d2528c8083d210f3022`；
- 镜像 contract：69 表、25 张视频表；关闭功能后的 profile：44 表、0 张视频表；
- profile 指纹：`ecd541a96eba65d43fba66f59834f53987818b03ea10298f981a3ab965002fbe`，与迁移前正式库一致；
- 视频开关：关闭；兼容镜像在迁移前基线 readiness 为 `ready`；
- 最新备份完成并完整恢复验收后，正式前向脚本再次执行成功；
- 最终正式库：69 表、25 张视频表、视频表总行数 0；
- `ReviewArtifact` 三个视频字段存在，历史非空关联 0；两个视频枚举值存在；
- full schema guard：`ready=True`、0 项差异，指纹为
  `36904c220803175dbb65dc38329f29b8d41f1386cac46f4992d8f77ad6653f40`；
- 活动 profile schema guard：`ready=True`；容器状态 `healthy`；公开 readiness 全项为 `ok`；
- 小说 192、章节 221、审核记录 215；无效索引、未验证视频约束、长事务均为 0；
- 最终临时 SQL 已删除，最新备份与受限权限迁移日志保留。

## 8. 后续门禁

本次正式迁移已经完成。后续启用视频功能前必须同时满足：

1. 单独更新启用 spec 和生产配置授权，不能把本次结构晋升视为功能开放授权；
2. 启用前再次验证 full contract、素材存储、模型配置、归属校验和生产 smoke；
3. 不迁移或复用 `novelwriterdev` 测试数据；
4. 一旦正式视频域产生数据，不再执行 empty-only destructive rollback。
