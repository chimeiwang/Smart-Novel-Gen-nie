...
def test_历史正文复审首次派发与恢复继续使用原v1提示词(role, dispatch_mode, operation) -> None:
    ...
    request = rehash_request(request.model_copy(update={
...
def test_首次派发不允许混合新旧复审引用() -> None:
    ...
    request = rehash_request(request.model_copy(update={
        "modelProfile": request.modelProfile.model_copy(update={
...
async def test_正文复审保持严格校验并记录实际失败路径(case, caplog) -> None:
    ...
    # 编辑复审的 pass + 7 条问题来自现场复现；null patch 仅为类型诊断的合成回归。
    ...
    if case == "valid_issues":
        ...
        pointer, keyword = (
            ("/findings", "maxItems") if case == "pass_with_seven_findings"
