...
export const WRITING_ACTION_PROMPTS: Record<Exclude<WritingProductActionKind, "open_artifacts">, string> = {
  plan_beat:
    "请为当前章节生成一份可应用的章节计划。请拆成场景节拍，明确每场的目标、冲突、涉及角色、伏笔、预估字数和验收标准。",
  write_draft:
    "请根据当前章节、已确认章节计划（如有）、大纲、设定和文风，生成当前章节正文草案。正文进入待审核草案，先不要直接写入正式章节。",
  review_chapter:
    "请从网文追读角度审核当前章节，重点看钩子、爽点、节奏、章节尾钩和读者承诺是否成立，并给出可执行修改建议。",
  consistency_check:
    "请检查当前章节正文与角色设定、世界规则、大纲和伏笔是否冲突，重点指出 OOC 和逻辑断裂。",
  rewrite_scene:
    "请找出当前章节最需要加强的一场戏并重写，重点补足目标、阻力、转折、代价和余波。重写内容进入待审核草案。",
  ooc_check:
    "请重点检查当前章节是否存在角色 OOC：对照角色核心欲望、行为边界、说话习惯、关系原则和短期目标，列出风险和修改建议。",
};
