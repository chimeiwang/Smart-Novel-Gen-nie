...
public final class WorkflowRunCancellationService {
    ...
    public void cancelDeletedStyle(WorkflowStylePortraitCompletion.DeletedRun run) {
        repository.requestDeletedStyle(run.userId(), run.runId(), run.cancelRequestId())
                .executorRequests().forEach(this::deliver);
    }

    public void cancelInvalidatedRag(WorkflowRagIndexCompletion.InvalidatedRun run) {
        repository.requestInvalidatedRag(run.userId(), run.runId(), run.cancelRequestId())
                .executorRequests().forEach(this::deliver);
    }
