...
- PostgreSQL schema 默认冻结。已批准例外包括
  ...
  数据库名校验，正式库执行还必须提供精确确认令牌；该授权不包含其他生产 DDL、手机号数据删除、老账号绑定
  或账号合并。任何其他持久化改动必须先更新 spec 和本文件、
...
- 当前商业化缺口包括邮箱、账号找回、在线支付、管理员后台、团队协作、移动端、内容发布分发；手机号认证代码、
  开发库迁移、公开协议与真实短信送达测试已完成，正式库迁移和生产启用已获批准但在受控执行完成前仍默认关闭。
...
- Java Core 已于 2026-08-26 单切生产并处于观察期：生产始终只有一个 Core，不双 Core、不双写；Python
  ...
  spec 实施且开发库具名迁移已完成；正式库迁移和生产启用已获得单独批准，必须在备份、迁移复验和健康门禁完成
  后才可宣告开放。
