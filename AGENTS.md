...
仓库级语言铁律：**仓库内所有自然语言代码注释、文档字符串、文档正文、计划、规格、审计、备注、示例说明和用户可见维护说明一律使用简体中文。新增或修改内容不得使用英文或繁体中文进行自然语言叙述。**代码标识符、协议字段、命令、文件路径、环境变量、第三方产品或库专名，以及无法改写的外部原文引用除外。历史文件只有在被修改时才必须同步清理其受影响部分，不要求一次性翻译全部归档内容。具体文档治理和验收规则以根目录 `DOCS.md` 为准，不另建第二套规范。
