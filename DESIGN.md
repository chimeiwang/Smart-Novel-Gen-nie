...
### Visual Canon Workspace

视觉设定属于章节影视化生产工作台，不做成营销式图片画廊。

- 继续使用“设定卡列表 / 候选与正式版本 / 逐镜绑定”三栏结构；
- 候选图与正式图必须同时显示文字状态，不能只靠边框颜色区分；
- 图片缩略图服务于版本核对，保持紧凑比例，不用大瀑布流或沉浸式封面；
- “上传候选”和“确认为视觉设定”是两个明确动作，批准按钮靠近候选图；
- 逐镜参考同时显示设定名称、职责、版本和强度，缺少角色或场景参考使用非阻断 warning；
- 提示词编辑器展示该候选或正式版本冻结的参考图，不用当前 Head 冒充历史来源。
