...
- 人工排查入口是 `logs/workflow-events/runs/YYYY-MM-DD/<task短号>.log`；同一 WritingTask 的初次运行和 resume 按发生时间连续追加，每次运行只写一次上下文头。
- 全文按实际发生顺序记录 workflow、完整可序列化 LangGraph 初始状态、节点完整 state patch、关键状态 diff、Operation、Agent、LLM 输入/输出和工具输入/返回，不需要跨日志文件关联。
...
- token stream、`on_chain_*`、Runnable 和 checkpoint namespace 等底层包装事件不进入人工日志。机器 JSONL 仅在 `WORKFLOW_MACHINE_EVENT_LOG_ENABLED=true` 时生成，默认关闭。
- GraphState 中 callback、函数、循环引用和 `UntrackedValue` 的 `novelData` 属于 runtime-only 数据，显式标记后排除；正文或普通 tracked state 不做静默截断。
