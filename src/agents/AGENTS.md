...
编辑/校验 get_active_review_artifact
  ↓
submit_evaluation(revise/block) → operationWorkflow 返工边
  ↓
主责 Agent 更新同一 artifactKey 的新 revision
  ↓
...
通用“评估者/返工者”循环使用 `submit_evaluation(pass | revise | block)` 表达审核结论和 `requiredChanges`，再由 `operationWorkflow` 的 conditional edge 决定返工、结束或等待用户确认；不要为每个新场景手写一套专用流程状态机。
...
- ModelRuntimePort：模型供应商适配，只做 `streamText`、`completeText`、`completeStructured`、`runToolCallTurn`；不得解析 `propose_updates`、`submit_evaluation` 等业务 control event。
