...
- 2026-07-05: v8.54 双记录工作流日志：保留单个 `<task短号>.log`，但每次运行只输出 LLM 原文区块和“LangGraph 中文状态切换”两个部分。停止在人读文件中重复输出 Workflow/SSE JSON、完整 GraphState/raw patch、Agent final 和 agent_done 正文；审批/丢弃等既不调用 LLM 也不运行 Graph 的短路操作不创建空壳人工日志；机器 JSONL 开关继续承担底层事件审计。
...
- 2026-07-05: v8.56 Workflow 人工 LLM 日志精简：`<task短号>.log` 的 LLM 部分只输出实际发送给模型的 messages 原文、模型返回的 content 原文和 token 消耗；不再展开 tools schema、供应商 reasoning、模型 tool_calls、工具参数和工具返回。底层审计继续由显式开启的机器 JSONL 承担。
...
- 每个运行区块在简短运行头之后只能有两个一级部分：`LLM 输入与输出原文`、`LangGraph 状态切换（中文）`。不得恢复调用顺序、事件时间线、状态索引和完整详情等平行阅读层。
...
- LLM 部分按 `Axx` 分组，只渲染 `REQUEST / RESPONSE`。`TOOL_CALL`、`AGENT_RUN_FINAL` 与 `agent_done.content` 不进入人工日志。
...
- LLM 输入保留实际发送的 messages 原文；LLM 输出保留模型正文原文和 token 消耗。人工日志不展开 tools schema、供应商 reasoning、模型 tool calls、工具解析参数或工具返回；需要这些底层审计数据时显式开启机器 JSONL 或分片 LLM 日志。
