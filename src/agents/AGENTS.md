...
- 配置：`langgraph.json`
- 图入口：`apps/agent-service/src/inkforge_agents/studio.py`
...
Python `StateGraph` 已接管 CreativeOperation 分类后的准备、主责执行、草案提交、复审
`Send` 扇出、补丁或重写、用户 `interrupt()` 与 `Command` 恢复。稳定快照使用版本信封
并拒绝运行时字段；所有草案副作用继续通过待接线的 Core 接口端口完成。
