...
- 2026-06-28: v8.32 待审核草案继续修改收口：前端“继续修改”只关闭审核弹窗并聚焦聊天输入；当任务仍在 `awaiting_user_review` 或快照含 `pendingUserResponse + activeArtifactId` 时，下一条普通聊天由 `/resume` 自动路由为同一草案的新 revision 返工请求。
...
- 用户继续修改：前端按钮只把焦点交还聊天输入，不直接提交 `artifact_review revise`。用户下一条普通聊天若发生在当前任务待审核草案期间，`/resume` 会根据 `generatedContent` 或 graph snapshot 中的 `activeArtifactId` 构造返工 brief，路由回 `updatedByAgent` / `createdByAgent`，并围绕同一 `artifactKey` 产生新 revision。
...
- ModelRuntimePort：模型供应商适配，只做 `streamText`、`completeText`、`completeStructured`、`runToolCallTurn`；支持 `ModelCallProfile` 区分 `normal` 与 `fast` 调用预算/推理策略；不得解析 `propose_updates`、`submit_evaluation` 等业务 control event。
...
当前系统已经使用 `StateGraph`、`Annotation.Root`、conditional edges、`interrupt()` 和 `MemorySaver`，主线结构是：
...
- `propose_updates`、`submit_evaluation`、`submit_quality_report`、`submit_beat_plan`、`submit_validation_report` 是 side-effect control tools，单独调用后允许模型继续整理回复。注意：在 `operationWorkflow` 的 `plan_chapter` 主路径中，`submit_beat_plan` 会创建结构化 `beat_plan` ReviewArtifact，并由执行器结束本次产物创建，不再回退为纯文本 Beat Plan 草案。
